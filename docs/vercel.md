...
云端版使用独立的 Upstash Redis 数据，不会上传或同步本地任务、路径和备份。首次运行创建空看板。云端支持项目、任务、Prompt、回收站、备份与恢复，以及 GitHub 远端分支、提交、标签、最新提交的文件变更；本机未提交修改、Worktree、Stash 和目录浏览仍只在本地使用。

## GitHub 远端状态

- 云端点击 **Git 状态** 即可查看远端。默认项目未填写仓库时，使用部署仓库（或 `GITHUB_REPOSITORY` 指定的 `owner/repo`）；其他项目在 **设置 → GitHub 仓库** 或项目编辑弹窗中填写 `owner/repo`、HTTPS GitHub URL 或 SSH clone URL。这里只支持 github.com。
- 公开仓库不需要 Token。私有仓库或需要更高 API 额度时，在 Vercel 添加 `GITHUB_TOKEN` 并重新部署。建议使用仅授权所需仓库、Contents 只读的 fine-grained Token。Token 不进入网页、看板数据或 GitHub 仓库。
- 远端视图按需刷新，不使用本地的 5 秒轮询。未配置 Token 时缓存最多 10 分钟，已配置时最多 1 分钟；页面显示实际读取时间。缓存仅存在于服务实例内，多实例共享 GitHub 出站 IP 时仍可能限流，届时会显示明确提示。
- 可切换远端分支，查看最近 30 条提交和最新提交的前 100 个变更文件；分支和标签列表最多显示 100 条。存在后续分页时会提示到 GitHub 查看完整内容。提交标题链接可打开完整提交详情。
- 云端项目沿用 `path` 字段保存仓库地址。本地依然将这个字段作为磁盘路径，两个环境的数据独立，不需要修改本地配置。
